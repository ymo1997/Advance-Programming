...
#define PERCENTAGE_THRESHOLD 0.25
#define NUM_THRESHOLD 10
...
//global variable for universal hash
int* a;
...
void resize_dictionary() {
    ...
    binsNum = tempBinsNum;
    if(expandFlag == 1) {
        if(hashMethodNum == 4) binsNum = nextPrime(binsNum * 2);
        else binsNum *= 2;
    }
    else{
        for(int i = 0; i < r; i++) a[i] = rand() % r;
    }
    ...
    for(int i = 0; i < count; i++) {
        ...
        if(previousPtr == NULL) dictionary[hashCode].head = newNode;
        else previousPtr->next = newNode;
...
int add_word(char * word, char * definition)
{
    ...
    if(previousPtr == NULL) {
        ...
        dictionary[hashCode].size = 0;
    }
    else{
        previousPtr->next = newNode;
    }
    dictionary[hashCode].size++;
    dictionary[hashCode].last = newNode;

    if(hashMethodNum == 4 && (dictionary[hashCode].size > NUM_THRESHOLD && 
        ((double)dictionary[hashCode].size / wordNum) > PERCENTAGE_THRESHOLD)) {
        expandFlag = 0;
        resize_dictionary();
    }
...
int main(int argc, char** argv)
{
    ...
    switch(hashMethodNum) {
        ...
        case(3):
            ...
            break;
        case(4):
            hashFunction = universal_hash;
            expandFlag = 1;
            r = nextPrime(WORD_LEN);
            binsNum = r;
